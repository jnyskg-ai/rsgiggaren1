...
(() => {
  ...
  function defaultPolicy() {
    ...
    if (/push/.test(day)) return rankMap(['chest','frontDelts','rearSideDelts','triceps','traps','biceps','forearms','other']);
  ...
  function moveExercise(slot, direction) {
    ...
    saveCurrentOrder(rows); workout();
    toast(direction < 0 ? 'Övningen flyttades upp' : 'Övningen flyttades ned');
  }
  function resetCurrentOrder() {
    saveWorkoutDraft(); ensureOrderStore(); delete D.exerciseOrder[orderKey()]; save(); workout();
    toast('Standardordningen är återställd');
  }
  function ensureOrderToolbar() {
    const list = document.querySelector('#exerciseList'); if (!list) return;
    let toolbar = document.querySelector('#exerciseOrderTools');
    if (!toolbar) { toolbar = document.createElement('div'); toolbar.id='exerciseOrderTools'; toolbar.className='why'; toolbar.style.margin='8px 0 12px'; list.parentNode.insertBefore(toolbar,list); }
    ...
    const reset=document.querySelector('#resetExerciseOrder'); if(reset) reset.onclick=resetCurrentOrder;
  }
  function decorateExerciseOrderButtons() {
    const exercises=[...document.querySelectorAll('#exerciseList .exercise')];
    exercises.forEach((exercise,index)=>{
      const actions=exercise.querySelector('.exercise-actions'); if(!actions||actions.querySelector('.order-up')) return;
      const up=document.createElement('button'); up.type='button'; up.className='btn smallbtn order-up'; up.textContent='↑'; up.title='Flytta övningen upp'; up.setAttribute('aria-label',`Flytta ${exercise.dataset.name} upp`); up.disabled=index===0; up.onclick=e=>{e.preventDefault();moveExercise(exercise.dataset.slot,-1)};
      const down=document.createElement('button'); down.type='button'; down.className='btn smallbtn order-down'; down.textContent='↓'; down.title='Flytta övningen ned'; down.setAttribute('aria-label',`Flytta ${exercise.dataset.name} ned`); down.disabled=index===exercises.length-1; down.onclick=e=>{e.preventDefault();moveExercise(exercise.dataset.slot,1)};
      actions.prepend(down); actions.prepend(up);
    });
  }

  const nativeWorkout=workout;
  workout=function orderedWorkout(){ nativeWorkout(); ensureOrderToolbar(); decorateExerciseOrderButtons(); };
  const nativeRender=render;
  render=function orderedRender(){ nativeRender(); const version=document.querySelector('#appVersion'); if(version) version.textContent=`RSG Coach ${ORDER_VERSION} • dataschema ${DATA_SCHEMA}`; ensureOrderToolbar(); decorateExerciseOrderButtons(); };
  ...
  if(saveWorkoutButton?.onclick){
    ...
    saveWorkoutButton.onclick=function(event){ const before=D.sessions.length; nativeSaveWorkout.call(this,event); if(D.sessions.length>before){ const session=D.sessions.at(-1); if(session) session.appVersion=ORDER_VERSION; save(); } };
  }
